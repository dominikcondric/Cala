...
void OGLShader::attachShader(ShaderType shaderStage, const std::filesystem::path& filePath)
{
...
void OGLShader::activate() const
{
	glUseProgram(programID);
}

void OGLShader::dispatchComputeShader(uint32_t workGroupX, uint32_t workGroupY, uint32_t workGroupZ) const
{
...
void OGLShader::attachConstantBuffer(ConstantBuffer* buffer, const std::string& bufferName) const
{
	if (programID == GL_NONE)
		return;
...
ConstantBuffer::ConstantBufferInfo OGLShader::getConstantBufferInfo(const std::string& bufferName) const
{
